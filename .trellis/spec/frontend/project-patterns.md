...
「原文 PDF」「文字提取」两栏支持最小化（`src/ui/paneCollapse.js`）。任一栏最小化时，所有 `.pane-gutter` 由 paneCollapse 加 `.pane-gutter--disabled`；`paneResize.js` 的 `mousedown` / `dblclick` 各加一句守卫跳过：

```js
if (gutterEl.classList.contains('pane-gutter--disabled')) return;
```

这样最小化期间无法拖拽（避免与「最小化栏 = 固定像素」的布局冲突），全部恢复后分隔条重新可拖，且行内 `style.width` 已是基准比例，拖拽数学不受影响。
...
- ❌ 最小化时仍允许拖拽分隔条（重分配的临时比例会被 `savePaneRatios` 误存，破坏基准比例）。
