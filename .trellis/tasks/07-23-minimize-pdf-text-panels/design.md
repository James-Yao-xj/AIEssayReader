...
`paneCollapse.applyLayout` 不再「任一栏最小化就禁用所有分隔条」，而是**精确禁用紧邻最小化竖条的分隔条**：

- 布局 `[pdf] gutter0 [text] gutter1 [ai]`。分隔条 `i` 的左右栏索引为 `i`、`i+1`。
- 该分隔条禁用当且仅当其左栏或右栏处于最小化态（`activeMin.has(PANE_KEYS[i])` 或 `has(PANE_KEYS[i+1])`）。
- 结果：仅最小化 PDF → gutter0 禁用、gutter1 可拖；仅最小化文字 → 两根均禁用（PDF 与 AI 被竖条隔开）；两栏都最小化 → 均禁用。
...
这样最小化时，剩余非紧邻分隔条仍可拖动调整占比；紧邻竖条的禁用态分隔条不响应。全部恢复后所有分隔条重新可拖。

### 持久化感知最小化（避免基准比例污染）

最小化栏的行内宽度是固定像素（`36px`），不是百分比。若 `mouseup` 直接 `parseFloat` 全部三栏存盘，会把 `36px` 当成 `36%` 污染基准比例，且恢复后三栏总和溢出容器。

因此 `paneResize.readCurrentRatios`（供 `mouseup` 持久化用）改为最小化感知：
- 最小化栏 → 取基准比例 `loadPaneRatios()[i]`（保留其展开后的绝对占比）。
- 非最小化栏 → 取当前百分比。
- 归一化：最小化栏保持基准绝对值不变，仅缩放非最小化栏，使三栏总和回到 `DEFAULT_RATIOS` 总和（约 99）。

效果（数值验证，容器 1200px，基准 [36,30,33]）：最小化 PDF → TEXT 45.71% / AI 50.29%；拖 gutter1 到 TEXT 50% / AI 46%；存盘得 [36, 32.81, 30.19]（PDF 不变，TEXT:AI 保留 50:46 相对比例，总和 99）；恢复 PDF 后三栏填满容器且用户的占比调整被保留。

注：`mousemove` 只调整被拖分隔条两侧的栏，最小化栏（不在该分隔条两侧）不受影响，拖拽数学无需改动。
...
- **基准比例污染**：最小化栏行内宽度是 `36px`，持久化时不能 `parseFloat` 当百分比。`readCurrentRatios` 已做最小化感知（最小化栏取基准、非最小化栏归一化），既不污染、恢复后也不溢出（见 §6）。
- **刷新后状态**：最小化状态本身不持久化（只存内存 Set），刷新后 `loadPaneRatios()` 返回用户最后拖拽保存的比例，三栏正常展开（满足 AC7）。
